...
From this scope the DHCP server can select the IP address to alocate to a new device.
...
Virtual Private Netwrok (VPN) allows devices to communicate to each other accross the network but encrypts all of the data that is sent over the network.

The **Concentrator** is the specialised device that encrypts and decrypts the data accross a VPN. Often integrated into a firewall.
...
Roter is a layer 3 device (network layer device)
...
Managed switches offer redundancy support (you can set up another switch for if the main managed swithc fails)

Managed swithces also offer port mirroring (capturing or monitoring network traffic)
...
Cables are permanently punched / terminated down to a patchpanel. Then from that patch pannel we use a short ethernet cable that connects to the apropriate slot on a switch.
